...
\end{equation}
where $Z(n)$ is the location of slice $n$ in mm relative to isocenter. Radial CAIPI phase blipping as described by Setsompop et al.~\cite{setsompopBlippedControlledAliasingParallel2012a} was implemented with the slice-specific phase blip $\phi_n$ constructed via Equation~\ref{eq:caipi_blips} 
\begin{equation}
...
\begin{table}[ht]
    ...
    Recon.\ spatial resolution (\qty{}{\mm^2}) & $0.94 \times 0.94^+$ & $1.41 \times 1.41$ & $1.40 \times 1.40$ & $1.40 \times 1.40$ \\
    Recon.\ cardiac frames & 40 & 40 & 40 \\
    ...
    \begin{flushleft}
     \footnotesize
        ...
        $^+$axial acquisition had recon.\ spatial resolution of $0.70 \times 0.70$ \qty{}{\mm^2}
...
Acquired ultrasound and MRI data were analyzed in MATLAB (Mathworks, Natick, MA) to compare ultrasound-derived vs MRI-derived PWVs. MRI data was analyzed using a custom-built analysis platform developed by Grant Roberts and myself in MATLAB (available at: \url{https://github.com/tnaren97/PWV_2DPC})~\cite{narenPWV_2DPC}. This tool consists of two different Graphical User Interfaces (GUIs), one for aortic centerline tracing to calculate $\Delta d$ and the second for time-shift calculations between velocity waveforms for $\Delta t$.
...
\begin{table}[ht]
    ...
    & TTP (m/s)  & 8.7 (11.8\%) & 8.3 (27.2\%) & 11.9 (11.3\%) \\
    & TTF (m/s)  & 9.7 (15.2\%) & 11.2 (25.0\%) & 16.5 (42.4\%) \\
    & TTU (m/s)  & 11.0 (\phantom{0}4.6\%) & 11.9 (14.8\%) & 11.9 (\phantom{0}9.3\%) \\
    & XCOR (m/s) & 6.2 (17.8\%) & 7.2 (25.9\%) & 10.0 (\phantom{0}8.0\%) \\
    ...
    & TTP (m/s)  & 10.6 (\phantom{0}7.3\%) & 10.6 (\phantom{0}7.4\%) & 13.3 (25.0\%) \\
    & TTF (m/s)  & 10.8 (29.2\%) & 12.2 (36.9\%) & 15.4 (32.5\%) \\
    & TTU (m/s)  & 11.2 (\phantom{0}6.2\%) & 9.7 (\phantom{0}5.8\%) & 9.6 (11.6\%) \\
    & XCOR (m/s) & 9.2 (21.8\%) & 9.6 (\phantom{0}0.4\%) & 8.3 (23.2\%) \\
    ...
    & TTP (m/s)  & 9.6 (\phantom{0}2.6\%) & 10.6 (\phantom{0}6.9\%) & 11.6 (\phantom{0}8.9\%)\\
    & TTF (m/s)  & 9.0 (\phantom{0}7.2\%) & 9.2 (\phantom{0}3.5\%) & 11.0 (\phantom{0}4.8\%) \\
    & TTU (m/s)  & 10.7 (\phantom{0}1.4\%) & 9.9 (\phantom{0}4.2\%) & 12.2 (11.8\%) \\
    & XCOR (m/s) & 7.0 (\phantom{0}6.9\%) & 8.7 (10.7\%) & 11.1 (\phantom{0}1.9\%) \\
    ...
    & TTP (m/s)  & 9.9 \phantom{(00.0\%)} & 11.4 \phantom{(00.0\%)} & 10.7 \phantom{(00.0\%)} \\
    & TTF (m/s)  & 8.4 \phantom{(00.0\%)} & 8.9 \phantom{(00.0\%)} & 11.6 \phantom{(00.0\%)} \\
    & TTU (m/s)  & 10.5 \phantom{(00.0\%)} & 10.3 \phantom{(00.0\%)} & 10.9 \phantom{(00.0\%)} \\
    & XCOR (m/s) & 7.6 \phantom{(00.0\%)} & 9.7 \phantom{(00.0\%)} & 10.9 \phantom{(00.0\%)} \\
    & Average (m/s) & 9.1\phantom{0} $\pm$ \phantom{0}1.4 & 10.1\phantom{0} $\pm$ \phantom{0}1.0 & 11.0\phantom{0} $\pm$ \phantom{0}0.4 \\
